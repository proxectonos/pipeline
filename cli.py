...
def build_parser(handlers: Dict[str, Callable[[argparse.Namespace], None]]) -> argparse.ArgumentParser:
    ...
    p.add_argument("-sl", "--source_lang", type=str, default="glg_Latn")
    p.add_argument("-tl", "--target_lang", type=str, default="eng_Latn")
    p.add_argument("-sc", "--scorer", choices=["blaser_qe", "cosine"], default="blaser_qe")
    p.add_argument("-d", "--device", type=str, default="cuda")
    p.add_argument("-bs", "--batch_size", type=int, default=256)
    p.add_argument("-bmt", "--batch_max_tokens", type=int, default=None)
    p.add_argument("-st", "--score_threshold", type=float, default=3.0)
    p.add_argument("-sm", "--shift_margin", type=float, default=0.10)
    p.add_argument("-nw", "--neighbor_window", type=int, default=2)
    p.add_argument("-mlr", "--min_length_ratio", type=float, default=0.30)
